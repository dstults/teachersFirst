				...
				<td>
					<#list day as openingData>
						<#if openingData?has_content>
							<br><a style="color: blue;" href="/make_appointment?instructor=${openingData.instructor}&startTime=${openingData.startTime}&endTime=${openingData.endTime}">${openingData.instructor}:<br>${openingData.startTime} - ${openingData.endTime}<br></a>
						<#else>
							No openings!
						</#if>
					</#list>
